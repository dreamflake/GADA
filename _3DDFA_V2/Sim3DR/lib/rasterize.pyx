...
@cython.boundscheck(False)  # turn off bounds-checking for entire function
@cython.wraparound(False)  # turn off negative index wrapping for entire function
def rasterize_adv(np.ndarray[float, ndim=3, mode = "c"] image not None,
              np.ndarray[int, ndim=3, mode = "c"] used_area not None,
              np.ndarray[int, ndim=1, mode = "c"] ref_triangles not None,
              np.ndarray[float, ndim=2, mode = "c"] vertices not None,
              np.ndarray[int, ndim=2, mode="c"] triangles not None,
              np.ndarray[float, ndim=2, mode = "c"] colors not None,
              np.ndarray[float, ndim=2, mode = "c"] depth_buffer not None,
    ...
    _rasterize_adv(
        <float*> np.PyArray_DATA(image), <int*> np.PyArray_DATA(used_area),
        <int*> np.PyArray_DATA(ref_triangles),<float*> np.PyArray_DATA(vertices),
        <int*> np.PyArray_DATA(triangles),
        <float*> np.PyArray_DATA(colors),
        <float*> np.PyArray_DATA(depth_buffer),
        ntri, h, w, c, alpha, reverse)
@cython.boundscheck(False)  # turn off bounds-checking for entire function
@cython.wraparound(False)  # turn off negative index wrapping for entire function
def get_image_color(np.ndarray[float, ndim=3, mode = "c"] image not None,
              np.ndarray[int, ndim=3, mode = "c"] used_area not None,
              np.ndarray[int, ndim=1, mode = "c"] ref_triangles not None,
              np.ndarray[float, ndim=2, mode = "c"] vertices not None,
              np.ndarray[int, ndim=2, mode="c"] triangles not None,
