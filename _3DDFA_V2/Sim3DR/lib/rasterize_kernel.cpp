...
// Complexity: < ntri * h * w * c
void _rasterize_adv(
        float *image,int *used_area, int *ref_triangles,  float *vertices, int *triangles, float *colors, float *depth_buffer,
        int ntri, int h, int w, int c, float alpha, bool reverse) {
    int x, y, k;
    int tri_p0_ind, tri_p1_ind, tri_p2_ind;
    Point p0, p1, p2, p;
...
//                            p_color = weight[0] * p0_color + weight[1] * p1_color + weight[2] * p2_color;
//                            if (reverse) {
//                                image[(h - 1 - y) * w * c + x * c + k] = image[(h - 1 - y) * w * c + x * c + k] + alpha * 1. * p_color);
////                                image[(h - 1 - y) * w * c + x * c + k] = (unsigned char) (255 * p_color);
//                            } else {
//                                image[y * w * c + x * c + k] =  image[y * w * c + x * c + k] + alpha * 1. * p_color);
////                                image[y * w * c + x * c + k] = (unsigned char) (255 * p_color);
//                            }
//                        }
//
...
//    }
    ...
    //                                image[y * w * c + x * c + k] = (unsigned char) (255 * p_color);
                                ...
                                used_area[y * w * c + x * c + k]=i+1;
//                                ref_triangles[i]=i+1;
                            }
//                            else
//                            {
//                                ref_triangles[i]=used_area[y * w * c + x * c + k];
//                            }
                        }
                        depth_buffer[y * w + x] = p_depth;
                    }
...
//            continue;
//        }
//        if (x_max < x_min && (x_min+1)<(w - 1)) {
//           x_max=x_min+1;
//        }
//        if (y_max < y_min && (y_min+1)<(h - 1)) {
//           y_max=y_min+1;
//        }
//        for (y = y_min; y <= y_max; y++) {
//            for (x = x_min; x <= x_max; x++) {
...
//                // and judge is_point_in_tri by below line of code
//                if (!(weight[2] >= 0 && weight[1] >= 0 && weight[0] > 0)) {
//                    if ((p0_depth <depth_buffer[y * w + x]+1&&p0_depth > depth_buffer[y * w + x]-1))
//                            ref_triangles[i]=used_area[y * w * c + x * c + k];
//                }
//
//            }
                    ...
                    if ((p_depth > depth_buffer[y * w + x])) {
                        for (k = 0; k < c; k++) {
                            if (used_area[y * w * c + x * c + k]==0)
                            {
                                ...
                                p_color=p0_color;
                                if (reverse) {
                                colors[c * tri_p0_ind + k]= image[(h - 1 - y) * w * c + x * c + k];
//                                    image[(h - 1 - y) * w * c + x * c + k] = (
//                                           max(min(image[(h - 1 - y) * w * c + x * c + k] + alpha * 255. * p_color,255.),0.));
    //                                image[(h - 1 - y) * w * c + x * c + k] = (unsigned char) (255 * p_color);
                                } else {
                                colors[c * tri_p0_ind + k]=image[y * w * c + x * c + k];
//     image[y * w * c + x * c + k] = (
//                                           max(min(image[y * w * c + x * c + k]+ alpha * 255. * p_color,255.),0.));
    //                                image[y * w * c + x * c + k] = (unsigned char) (255 * p_color);
                                }
                                used_area[y * w * c + x * c + k]=i+1;
//                                ref_triangles[i]=i+1;
