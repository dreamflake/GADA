...
class Evolutionary_Geo_Attack(object):
    ...
    def __init__(self, model, dict_model, use_geo=True, use_dict=True, dimension_reduction=None,
                 random_background=False, only_one=False, restrict_face=False):
        # restrict_face argument is for EAF and EAFD
        self.model = model
        ...
        self.restrict_face=restrict_face
        ...
        if random_background:
            if torch.min(self.model.get_num_queries(torch.arange(0, x.size(0)))) % 20 != 0:
                if self.use_geo == True:
                    noised_x = (x + (torch.randn_like(x).cuda() * 0.04) * self.x_back_mask*0.25/self.x_back_mask.mean())
                else:
                    noised_x = (x + torch.randn_like(x).cuda() * 0.02)
            else:
                noised_x = x
            if targeted:
    ...
    def attack(self, x, y, x_s=None, targeted=False, max_queries=1000, total_search=False):
        ...
        face_features = self.dict_model.get_features(x.cuda())
        b = x.size(0)
        ...
        if self.use_geo == True:
            ...
            if targeted:
                ...
                if total_search:
                    ...
                    if self.visualize == True:  # For visualization
                        ...
                        plot_img(x_a_uv * 5 + 0.5,
                                 'x_a_' + str(self.count))

                self.x_back_mask = 1 - self.geoDict.project_adv_perturbation(x, ver_lst, depth_lst,
                                                                             torch.ones_like(x_a_ncc))
                self.x_back_mask = self.x_back_mask.cuda()
                # if self.b_std is None:
                #     self.b_std=(1-self.x_back_mask).mean()
                # else:
                #     self.b_std=(self.b_std+(1-self.x_back_mask).mean())


                # print('face:',self.b_std/self.count)
                iters = 0
            ...
            if self.restrict_face==True:
                if targeted:
                    ver_lst, depth_lst = self.geoDict.get_face_alignment(x_s)
                    original_ncc_codes, used_original_uv_codes = self.geoDict.get_image_color(x_s, ver_lst, depth_lst)
                    ones_ncc_codes = torch.ones_like(original_ncc_codes)
                    source_face_mask = self.geoDict.project_adv_perturbation(torch.zeros_like(x), ver_lst, depth_lst,
                                                                      ones_ncc_codes)
                else:
                    ver_lst, depth_lst = self.geoDict.get_face_alignment(x)
                    original_ncc_codes, used_original_uv_codes = self.geoDict.get_image_color(x, ver_lst, depth_lst)
                    ones_ncc_codes=torch.ones_like(original_ncc_codes)
                    face_mask = self.geoDict.project_adv_perturbation(torch.zeros_like(x), ver_lst, depth_lst, ones_ncc_codes)


            # initialize
            if targeted:
                if self.restrict_face==True:
                    x_a=source_face_mask*x_s+(1-source_face_mask)*x
                    check = self.is_adversarial(x_a, y, targeted, num_indices)

                    if check.sum() < y.size(0):
                        self.err_count += 1
                        print('Some initial images do not belong to the target class! Sum : ' +str(self.err_count) )
                        x_a = x_s
                    check = self.is_adversarial(x_a, y, targeted, num_indices)
                    if check.sum() < y.size(0):
                        print('Some initial images do not belong to the target class!')
                ...
                if self.restrict_face:
                    ...
                    while check.sum() < np.shape(y)[0]:
                        ...
                        #     x_a[check == False]) * iters / self.num_trial
                        x_a[check == False] = x[check == False] + (x_a[check == False] - x[check == False]) * 1.05
                        x_a[check == False] = x_a[check == False].clamp(0.2, 0.8)
                        x_a[check == False] = face_mask[check == False] * x_a[check == False] + (1 - face_mask[check == False]) * x[check == False]
                        check = self.is_adversarial(x_a, y, targeted, num_indices)
                        iters += 1
                        if iters > self.num_trial:
                            print('Initialization failed!')
        ...
        if background_attack_batch_size > 0:
            ...
            x_a_img = x_a.clone()
            if self.restrict_face and targeted==False:
                diagonal_covariances_img = diagonal_covariances_img * torch.nn.functional.upsample_bilinear(face_mask,
        ...
        if b - background_attack_batch_size > 0:
            ...
            # if self.u_std is None:
            #     self.u_std = x_a_uv_mask.mean()
            # else:
            #     self.u_std = (self.u_std + x_a_uv_mask.mean())

            # print('uv :', self.u_std / self.count)
            x_a_uv_mask[:, :, :20, :] = 1
        ...
        while torch.min(self.model.get_num_queries(num_indices)) < max_queries:
            ...
            for b_c in torch.arange(0, b)[background_attack_indices].long():
                if self.visualize == True:  # For visualization
                    if Q[b_c] % 1000 == 0:
                        if self.use_geo == False:
                            ...
                            plot_img((x_a_img[background_attack_indices] - x[background_attack_indices]) * 5 + 0.5,
                ...
                if Q[b_c] < max_queries:
                    ...
                    if self.restrict_face and targeted==False:
                        x_a[b_c] = face_mask[b_c] * x_a[b_c] + (
                                1 - face_mask[b_c]) * x[b_c]
